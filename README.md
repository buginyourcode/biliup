...
基于biliup开发的批量视频上传工具，支持扫描本地文件夹并批量上传视频文件到B站。
...
# 2. 扫描视频文件（可选）
python batch_bilibili_uploader.py /path/to/videos --scan-only

# 3. 批量上传视频
python batch_bilibili_uploader.py /path/to/videos

# 4. 并发上传（3个线程）
python batch_bilibili_uploader.py /path/to/videos -t 3
...
详细使用说明请参考：[README_BatchUploader.md](README_BatchUploader.md)
